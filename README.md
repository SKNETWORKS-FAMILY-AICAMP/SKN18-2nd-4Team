...
- **선수 검색**: 이름, 포지션, 클럽별 선수 검색
- **이적 예측**: AI 기반 이적 가능성 예측 (0-100%)
- **상세 분석**: 선수별 성능 지표 및 시장 가치 분석
- **시각화**: 예측 결과 및 성능 지표 차트 제공
- **실시간 업데이트**: 최신 데이터 기반 예측 결과 제공
...
```
├── train_df.csv             # 훈련 데이터 (12/13-21/22 시즌)
├── valid_df.csv             # 검증 데이터 (22/23 시즌)
├── test_df.csv              # 테스트 데이터 (23/24 시즌)
├── pred_df.csv              # 예측 데이터 (24/25 시즌)
```
...
- **Train Pool (12/13-23/24)**: 모델 학습 및 평가용
  - Train (12/13-21/22): 모델 학습
  - Valid (22/23): 하이퍼파라미터 튜닝
  - Test (23/24): 최종 성능 평가
- **Prediction (24/25)**: 실제 예측 대상

### 핵심 원칙

- **시계열 순서**: 과거 → 현재 → 미래 순서 유지
- **데이터 누수 방지**: 각 시즌별로 독립적인 데이터 사용
- **실용적 접근**: 23/24 데이터로 24/25 예측 (가장 최근 데이터 활용)
...
- **모델 특징**: 8개 모델을 1차 학습 후 성능 비교, 상위 3개 모델에 대해 하이퍼파라미터 튜닝, 정규화 및 앙상블 수행
