...
### 프로젝트 소개 및 기대효과
- 프리미어리그는 세계에서 인기 있는 리그 중 하나로,  
팬들과 분석가들이 선수 정보를 손쉽게 탐색하고 미래 이적 가능성을 확인할 수 있는 플랫폼의 수요가 큽니다.  
이 프로젝트는 **실시간 선수 탐색 + AI 기반 이적 예측**을 제공하는 **웹 애플리케이션**을 목표로 합니다.
...
#### **1️⃣ 하이퍼파라미터 튜닝 결과 (2단계 진행)**

| 모델                | 튜닝 전 | 1단계 후   | 2단계 후 | 최종 최적값 | 최적 파라미터                         |
| ------------------- | ------- | ---------- | -------- | ----------- | ------------------------------------- |
| Logistic Regression | 0.4651  | **0.4678** | 0.4641   | **0.4678**  | C=1.0, penalty='l2'                   |
| LightGBM            | 0.4626  | **0.4626** | 0.3983   | **0.4626**  | 기본값 유지 (튜닝 실패)               |
| SVM                 | 0.4695  | **0.4695** | 0.4685   | **0.4695**  | C=0.1, kernel='linear', gamma='scale' |

**🏆 하이퍼파라미터 튜닝 최고 모델: SVM (Linear) 0.4695**

#### **2️⃣ 정규화 개선 결과 (완료)**

| 모델                     | 정규화 전 | 정규화 후  | 향상도      | 적용 기법             |
| ------------------------ | --------- | ---------- | ----------- | --------------------- |
| Logistic Regression (L1) | 0.4678    | **0.4631** | **-0.0047** | L1 Regularization     |
| Logistic Regression (L2) | 0.4678    | **0.4689** | **+0.0011** | L2 Regularization     |
| SVM (RBF)                | 0.4695    | **0.4744** | **+0.0049** | RBF Kernel + C Tuning |
| LightGBM (Regularized)   | 0.4626    | **0.4465** | **-0.0161** | reg_alpha, reg_lambda |

**🏆 정규화 최고 모델: SVM (RBF) 0.4744**

#### **3️⃣ 앙상블 모델링 결과 (완료)**

| 앙상블 방법   | 개별 모델 최고점수 | 앙상블 점수 | 향상도      | 구성 모델                |
| ------------- | ------------------ | ----------- | ----------- | ------------------------ |
| Bagging (RF)  | 0.4744             | **0.5093**  | **+0.0349** | Random Forest 앙상블     |
| Voting (Soft) | 0.4744             | 0.4876      | +0.0132     | SVM, LR, LightGBM        |
| Voting (Hard) | 0.4744             | 0.4625      | -0.0119     | SVM, LR, LightGBM        |
| Stacking      | 0.4744             | 0.4292      | -0.0452     | SVM, LR, LightGBM + Meta |
| Bagging (GB)  | 0.4744             | 0.4184      | -0.0560     | Gradient Boosting 앙상블 |

**🏆 앙상블 최고 모델: Bagging (RF) 0.5093**
...
| ----------------------- | ------------------- | --------- | ----------------- | ----------- |
| **기준선**              | SVM                 | 0.4695    | -                 | -           |
| **하이퍼파라미터 튜닝** | Logistic Regression | 0.4710    | **+0.0059**       | **+1.26%**  |
