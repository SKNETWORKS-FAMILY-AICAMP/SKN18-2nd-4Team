...
- **복합 점수**: 0.5604
- **선택 기준**: 복합 점수 (균등 가중 평균)
- **가중 공식**: (Accuracy + Precision + Recall + F1 + AUC) ÷ 5

**💡 균등 가중공식 채택 이유:**

- **균형잡힌 평가**: 단일 지표(F1)만으로는 모델의 전체적 성능을 판단하기 어려움
...
```
복합 점수 = (Accuracy + Precision + Recall + F1-Score + AUC) ÷ 5
```
...
| ---- | ----------------------- | ---------- | -------- | --------- | ------ | -------- | ------ |
| 🥇   | **Logistic Regression** | **0.5604** | 0.4736   | 0.3434    | 0.8581 | 0.4905   | 0.6365 |
| 🥈   | **SVM**                 | **0.5376** | 0.5649   | 0.3688    | 0.6659 | 0.4747   | 0.6139 |
| 🥉   | **LightGBM**            | **0.5215** | 0.6020   | 0.3801    | 0.5515 | 0.4500   | 0.6238 |
...
| ------------------- | ------- | ---------- | ----------- | ----------------------------------------------------- |
| Logistic Regression | 0.5604  | **0.5655** | **+0.0051** | {'C': 1.0, 'penalty': 'l1', 'solver': 'liblinear'}... |
| SVM                 | 0.5376  | **0.4320** | **-0.1056** | {'C': 0.1, 'gamma': 'scale', 'kernel': 'rbf'}...      |
| LightGBM            | 0.5215  | **0.4408** | **-0.0807** | {'learning_rate': 0.1, 'n_estimators': 300}...        |
...
| ----------------------- | ------------------- | --------- | ----------------- | ----------- |
| **기준선**              | Logistic Regression | 0.5604    | -                 | -           |
| **하이퍼파라미터 튜닝** | Logistic Regression | 0.5655    | **+0.0051**       | **+0.91%**  |
